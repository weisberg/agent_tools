...
fn run(ctx: &Context, command: Commands) -> Result<Value> {
    match command {
        ...
        Commands::Daemon(cmd) => daemon(ctx, cmd),
...
fn local(ctx: &Context, cmd: LocalCommand) -> Result<Value> {
    ...
    match cmd {
        ...
        LocalCommand::Search { query, semantic } => {
            ...
            Ok(
                json!({"query": query, "semantic": semantic, "backend": if semantic { "sqlite-vector-abstraction" } else { "sqlite-fts5" }, "data": results}),
            )
        }
...
fn daemon(ctx: &Context, cmd: DaemonCommand) -> Result<Value> {
    let socket = ctx.home.join("jirali.sock");
    match cmd {
        DaemonCommand::Start => Ok(json!({
            "daemon": "jiralid",
            "status": "not_started",
            "socket": socket,
            "fallback": "stateless",
            "message": "Daemon IPC protocol is declared; stateless execution remains active until the background server is installed."
        })),
        DaemonCommand::Stop => Ok(json!({"daemon": "jiralid", "status": "not_running"})),
        DaemonCommand::Status | DaemonCommand::Ping => Ok(json!({
            "ok": true,
            "daemon_available": socket.exists(),
            "fallback": "stateless",
            "socket": socket
        })),
    }
    ...
    for event in MarkdownParser::new_ext(markdown, options) {
        match event {
            ...
            Event::End(TagEnd::Link) => {
                push_text(&mut text_buffer, &mut inline, &marks);
                marks.retain(|m| !m.starts_with("link:"));
            }
            Event::Text(text) => {
    ...
    for mark in marks {
        match *mark {
            ...
            value if value.starts_with("link:") => {
                adf_marks.push(
                    json!({"type": "link", "attrs": {"href": value.trim_start_matches("link:")}}),
                );
            }
            _ => {}
        }
...
fn tools_schema() -> Value {
    json!({
        ...
        "groups": [
            ...
            "daemon", "apply", "batch", "diff", "snapshot", "api", "graphql", "mask"
        ],
        "implementation_status": {
            ...
            "daemon": "declared status/start/stop/ping surface with stateless fallback; background server process pending",
