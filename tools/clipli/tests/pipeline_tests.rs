...
fn test_html_to_j2_to_html_round_trip() {
    ...
    let step1 = clipli()
        ...
        .output()
        .unwrap();
    assert!(step1.status.success(), "step 1 (html->j2) failed: {}", String::from_utf8_lossy(&step1.stderr));
    let j2_output = String::from_utf8(step1.stdout).unwrap();
    assert!(j2_output.contains("{{"), "expected j2 template with placeholders, got: {j2_output}");
    ...
    let step2 = clipli()
        ...
        .output()
        .unwrap();
    assert!(step2.status.success(), "step 2 (j2->html) failed: {}", String::from_utf8_lossy(&step2.stderr));
    let html_output = String::from_utf8(step2.stdout).unwrap();
    assert!(html_output.contains("$5,678"), "expected rendered HTML to contain '$5,678', got: {html_output}");
}
...
fn test_table_render_to_plain_text() {
    ...
    let step1 = clipli()
        ...
        .output()
        .unwrap();
    assert!(step1.status.success(), "step 1 (paste table) failed: {}", String::from_utf8_lossy(&step1.stderr));
    ...
    let step2 = clipli()
        ...
        .output()
        .unwrap();
    assert!(step2.status.success(), "step 2 (html->plain) failed: {}", String::from_utf8_lossy(&step2.stderr));
    let plain_output = String::from_utf8(step2.stdout).unwrap();

    assert!(plain_output.contains("Alice"), "plain text should contain 'Alice', got: {plain_output}");
    assert!(plain_output.contains("Bob"), "plain text should contain 'Bob', got: {plain_output}");
    assert!(plain_output.contains("95"), "plain text should contain '95', got: {plain_output}");
    assert!(plain_output.contains("87"), "plain text should contain '87', got: {plain_output}");
}
...
fn test_templatize_preserves_rendering() {
    ...
    assert!(rendered_html.contains("Q1 2024"), "rendered HTML should contain 'Q1 2024', got: {rendered_html}");
    assert!(rendered_html.contains("$1,000"), "rendered HTML should contain '$1,000', got: {rendered_html}");
    assert!(rendered_html.contains("10%"), "rendered HTML should contain '10%', got: {rendered_html}");
}
...
fn test_convert_chain_html_plain() {
    ...
    let output = clipli()
        ...
        .output()
        .unwrap();
    assert!(output.status.success(), "html->plain failed: {}", String::from_utf8_lossy(&output.stderr));
    let plain = String::from_utf8(output.stdout).unwrap();

    assert!(plain.contains("Product"), "plain text should contain 'Product', got: {plain}");
    assert!(plain.contains("Price"), "plain text should contain 'Price', got: {plain}");
    assert!(plain.contains("Widget"), "plain text should contain 'Widget', got: {plain}");
    assert!(plain.contains("$9.99"), "plain text should contain '$9.99', got: {plain}");
}
...
fn test_multiple_j2_renders_same_template() {
    ...
    let render1 = clipli()
        .args([
            "convert", "--from", "j2", "--to", "html",
            "-D", r#"{"greeting":"Hello","name":"Alice"}"#,
        ])
        .write_stdin(template)
        .output()
        .unwrap();
    assert!(render1.status.success(), "render 1 failed: {}", String::from_utf8_lossy(&render1.stderr));
    let output1 = String::from_utf8(render1.stdout).unwrap();
    assert!(output1.contains("Hello, Alice!"), "expected 'Hello, Alice!' in output, got: {output1}");

    // Render with second dataset
    let render2 = clipli()
        .args([
            "convert", "--from", "j2", "--to", "html",
            "-D", r#"{"greeting":"Hi","name":"Bob"}"#,
        ])
        .write_stdin(template)
        .output()
        .unwrap();
    assert!(render2.status.success(), "render 2 failed: {}", String::from_utf8_lossy(&render2.stderr));
    let output2 = String::from_utf8(render2.stdout).unwrap();
    assert!(output2.contains("Hi, Bob!"), "expected 'Hi, Bob!' in output, got: {output2}");
}
...
fn test_paste_table_default_produces_valid_html() {
  ...
  "style": {"header_bg":"#1F497D","header_fg":"#FFFFFF"}
    ...
    let output = clipli()
        ...
        .output()
        .unwrap();
    assert!(output.status.success(), "paste table failed: {}", String::from_utf8_lossy(&output.stderr));
